...
export class InviteDoctorDialog {
  ...
  protected get targetRole(): UserRole {
    return this.authService.currentDoctor?.role === 'admin' ? 'doctor' : 'assistant';
  }

  protected get subjectLabel(): string {
    return this.targetRole === 'doctor' ? 'doctor' : 'asistente';
  }

  protected get title(): string {
    return this.targetRole === 'doctor' ? 'Invitar doctor' : 'Invitar asistente';
  }
  ...
  async save() {
    ...
    try {
      ...
      await setDoc(doc(this.firebase.firestore, 'users', uid), {
        ...
        pending: true,
        createdBy: this.authService.currentDoctor?.uid ?? '',
