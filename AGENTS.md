...
- `src/app/core/models/notification.ts`: `AppNotification`, `NotificationType`, `NotificationRecipientStatus`
- `src/app/core/repositories/notification.repository.ts`: `create()`, `watchForRecipient()`, `markRead()`/`markAllRead()`
- `src/app/core/services/notification.service.ts`: genera notificaciones (created/cancelled/rescheduled) + `buildRecipients()` (dueño + asistentes + familia del paciente, con fallback asistente→doctor, admin excluido) + `addPatientFamily()`
...
- `src/app/shared/components/notifications-dialog/`: diálogo de altura dinámica (`height auto; min-height 168px; max-height min(560px, calc(100dvh - 88px))`), header+filtros fijos, scroll SOLO en `.notifications-list`; abrir NO marca leídas; "Ver detalles" es la única acción que llama `markRead(notificationId, recipientId)`, luego navega; el badge permanece si quedan no-leídas
